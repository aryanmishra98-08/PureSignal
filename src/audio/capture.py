...
frame_queue: queue.Queue = queue.Queue(maxsize=500)  # ~10s of 20ms frames
...
def _capture_callback(indata: np.ndarray, frames: int, time_info, status) -> None:
    ...
    try:
        ...
        pass  # processing is falling behind; drop oldest frame
...
def start_capture() -> sd.InputStream:
    ...
    try:
        ...
        raise RuntimeError(
            f"[capture] Failed to open microphone: {e}\n"
            "  Check that a microphone is connected and accessible."
        ) from e
    print(
        f"[capture] mic open — {config.SAMPLE_RATE}Hz, "
